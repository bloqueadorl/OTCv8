...
addIcon("AmpRes", {item={id = 3278, count = 1}, hotkey=nil, movable=true, text="AmpRes"},
macro(100, "", function()
if not timecast or timecast <= now then
 for _, mob in ipairs(getSpectators(posz())) do
  if mob:isMonster() and not mob:isNpc() then
  local actualDist = getDistanceBetween(player:getPosition(), mob:getPosition())
   if actualDist >= 2 and actualDist <= 7 then
    if voc() == 1 or voc() == 11 then
     if modules.game_cooldown.isCooldownIconActive(237) or modules.game_cooldown.isGroupCooldownIconActive(3) then return end
     if mana() >= 80 then
      cast("exeta amp res")
      timecast = now + 2100
      return
    ...
    elseif voc() == 2 or voc() == 12 then
     if modules.game_cooldown.isCooldownIconActive(238) or modules.game_cooldown.isGroupCooldownIconActive(3) then return end
     if mana() >= 80 then
      cast("exana amp res")
      timecast = now + 16000
      return
...
local staminass = macro(500, "STAMINA POTION", function()
  if stamina() <= 2400 then
    for i, itemId in ipairs(itemIds) do
      if player:getItemsCount(itemId) > 0 then
        use(itemId)
        break
      end
    end
...
addIcon("staminass", {text = "S-Refill", item = 36725}, staminass)


  setDefaultTab("Main")
local panelName = "antiPushPanel"
local ui = g_ui.createWidget("ItemsPanel", panel)
ui:setId(panelName)
...
if type(storage[panelName].items) ~= 'table' then
    storage[panelName].items = {3031, 0, 0, 0, 0}
end

for i = 1, 5 do
    ui.items:getChildByIndex(i).onItemChange =
        function(widget) storage[panelName].items[i] = widget:getItemId() end
...
macro(100, function()
    ...
    if type(storage[panelName].lastItem) ~= 'number' or
        storage[panelName].lastItem > #candidates then
...
local reUse = addIcon("ClickReuseXD", {item =34079, text = "C-ReUse",  }, reUse )

addIcon("Anti Push", {text="AntPush", movable=true, hotkey = "l"}, function(self, isOn)
    self.item:setItemId(isOn and 30183 or 30184)
...
local exura_gran = "exura gran"
local exura_vita = "exura san"
local exura_max_vita = "exura gran san"
local min_hp_for_exura_gran = 99
local min_hp_for_exura_vita = 94
local min_hp_for_exura_max_vita = 85
local exhausted = 10 -- standardowy czas dla wszystkich czarów

local time = 0 -- nie zmieniac

macro(100, "Healing Macro", function()
  if time > now then return end
  
  local current_hp = hppercent()
  
  if current_hp <= min_hp_for_exura_max_vita then
    time = now + exhausted  -- 6 sekund dla czaru "exura max vita"
    say(exura_max_vita)
  elseif current_hp <= min_hp_for_exura_vita then
    time = now + exhausted 
    say(exura_vita)
  elseif current_hp <= min_hp_for_exura_gran then
    time = now + exhausted 
    say(exura_gran)
  end
end)
